...
use cogno_core::{
    validate_proposal, DataClassification, EvidenceOrigin, InputOrigin, ProposalAction,
...
fn corpus_deduplicates_by_fingerprint() {
    ...
    let added1 = add_internal(&mut c, a.clone());
    let added2 = add_internal(&mut c, a);
    assert!(added1);
...
fn add_internal(corpus: &mut Corpus, example: LabeledExample) -> bool {
    corpus
        .add_classified(example, DataClassification::Internal)
