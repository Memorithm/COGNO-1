...
use crate::mlp::{
    MlpNeuralModel, MAX_MLP_HIDDEN_FEATURES, MIN_MLP_HIDDEN_FEATURES,
};
use crate::neural::{
...
pub fn load_mlp_neural_artifact(
    ...
    let input_hidden = decode_f32s(
        bytes,
        &mut offset,
        layout.input_hidden,
        &mut scalar_index,
    )?;
    let hidden_bias = decode_f32s(
        bytes,
        &mut offset,
        layout.hidden_bias,
        &mut scalar_index,
    )?;
    let hidden_output = decode_f32s(
        bytes,
        &mut offset,
        layout.hidden_output,
        &mut scalar_index,
    )?;
    let output_bias = decode_f32s(
        bytes,
        &mut offset,
        layout.output_bias,
        &mut scalar_index,
...
mod tests {
    ...
    fn hostile_hidden_dimension_fails_before_weight_allocation() {
        let model = trained_model();
        let mut artifact = encode_mlp_neural_artifact(&model, 2_048).expect("encode");
        artifact.bytes[18..22]
            .copy_from_slice(&(u32::try_from(MAX_MLP_HIDDEN_FEATURES + 1).expect("u32")).to_le_bytes());
        artifact.manifest.weights_hash = sha256(&artifact.bytes);
        assert!(matches!(
            load_mlp_neural_artifact(&artifact.manifest, &artifact.bytes),
